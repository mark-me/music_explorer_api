...
""" lst_communities_data = []
community_max = 0
for graph in  tqdm(lst_graphs, total=len(lst_graphs)):
    graph = graph.copy()
    graph.vs['id_community_from'] = [community_max + 1] * len(graph.vs) 
    print("Number of vertices :" + str(len(graph.vs)) + "\nNumber of collection: " + str(sum(graph.vs['in_collection'])))
    if sum(graph.vs['in_collection']) > 1:
        df_communities = cluster_artist_graph(graph=graph)
    else:
        qty_vertices = len(graph.vs)
        df_communities = pd.DataFrame({'id_artist': graph.vs['name'],
                                        'name_artist': graph.vs['name_artist'],
                                        'in_collection': graph.vs['in_collection'],
                                        'id_hierarchy': [0] * qty_vertices,
                                        'id_community_from': [community_max + 1] * qty_vertices,
                                        'id_community': [community_max + 1] * qty_vertices,
                                        'eigenvalue': graph.vs['in_collection']})
    lst_communities_data.append(df_communities)
    community_max = max(df_communities['id_community']) 

df_data = pd.concat(lst_communities_data, axis=0, ignore_index=True)
...
df_communities = cluster_artist_graph(graph=graph)            # Start point for tree probing
db_writer = _db_writer.ArtistNetwork(db_file=db_file)
